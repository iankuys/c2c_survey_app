...
EMAIL_TEMPLATE_PATH = Path(".", "content", "reminder_email.html")
...
def construct_message_contents(user_email_addr: str, subject: str, key: str) -> tuple[str, str]:
    ...
    """
    with open(EMAIL_TEMPLATE_PATH) as infile:
        # HTML
...
def construct_message(
    ...
    if "@" not in from_addr:
        raise ValueError(f"Malformed 'from' email (missing '@'): {from_addr}")
    from_addr_parts = from_addr.split("@")
    if len(from_addr_parts) != 2:
        raise ValueError(f"Malformed 'from' email (incomplete address): {from_addr}")
    if "." not in from_addr_parts[1]:
        raise ValueError(f"Malformed 'from' email (bad domain): {from_addr}")
...
def send_mail(
    ...
    print(
        f"[{user_email_address}] sending access key '{access_key}' FROM '{our_email_address}'@'{our_smtp_server_address}'"
    )
    ...
    try:
        ...
        print(
            f"[{user_email_address}] Failed to send email (via {our_smtp_server_address}:{PORT})"
        )
        raise e
        # print(repr(e))
    return
